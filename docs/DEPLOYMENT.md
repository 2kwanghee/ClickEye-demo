...
## A. 서버 1회 구성 (EC2당 한 번) — Ubuntu 기준

### A-1. Docker + git 설치
```bash
ssh -i your-key.pem ubuntu@13.237.170.70
...
# nginx/apache 가 잡고 있으면: sudo systemctl disable --now nginx apache2 2>/dev/null
```
...
| Caddy가 80에서 안 뜸 | nginx/apache 잔존 → `sudo systemctl disable --now nginx apache2` (§A-2) |
